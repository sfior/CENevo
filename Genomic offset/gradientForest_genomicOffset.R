##### This script performs gradient forest modelling to predict compositional shifts in whole-genome adaptive genetic variation between current and future time, i.e., the genomic offset #####
...
# We'll plot a biplot encompassing the PC space of both present + future projections
PC1_comb <- c(PCs$x[, 1], PCs.future.df$PC1)
PC2_comb <- c(PCs$x[, 2], PCs.future.df$PC2)
PC3_comb <- c(PCs$x[, 3], PCs.future.df$PC3)
# Scale between 0-255. Use same standardisation scale as above.
r.biplot <- (PC1_comb - min(PCs.rast[[1]]@data@min, PCs.future.rast[[1]]@data@min)) / (max(PCs.rast[[1]]@data@max, PCs.future.rast[[1]]@data@max)-min(PCs.rast[[1]]@data@min, PCs.future.rast[[1]]@data@min))*255
g.biplot <- (PC2_comb - min(PCs.rast[[2]]@data@min, PCs.future.rast[[2]]@data@min)) / (max(PCs.rast[[2]]@data@max, PCs.future.rast[[2]]@data@max)-min(PCs.rast[[2]]@data@min, PCs.future.rast[[2]]@data@min))*255
b.biplot <- (PC3_comb - min(PCs.rast[[3]]@data@min, PCs.future.rast[[3]]@data@min)) / (max(PCs.rast[[3]]@data@max, PCs.future.rast[[3]]@data@max)-min(PCs.rast[[3]]@data@min, PCs.future.rast[[3]]@data@min))*255
...
scal <- 40
xrng <- range(PC1_comb, PCs$rotation[, 1]/scal) * 1.1
yrng <- range(PC2_comb, PCs$rotation[, 2]/scal) * 1.1
...
plot(PC1_comb, PC2_comb, xlim = xrng, ylim = yrng, pch = ".", cex = 4, col = rgb(r.biplot, g.biplot, b.biplot, max = 255), asp = 1, xaxt="n", yaxt="n")
...
axis(1, pos=0)
axis(2, pos=0)
# Add contours circumscribing the PC space of the present and future projections respectively
# Define hull method for masks
hull_method <- "concave"
# Make matrix of PC1 and PC2
PC12.present.matrix.temp <- cbind(PCs.df$PC1, PCs.df$PC2)
PC12.future.matrix.temp <- cbind(PCs.future.df$PC1, PCs.future.df$PC2)
# Generate hull around PC1 and PC2 coordinates
if (hull_method == "convex") {
  # Convex hull
  PC12.present.convexHull <- chull(PC12.present.matrix.temp)
  PC12.future.convexHull <- chull(PC12.future.matrix.temp)
  PC12.present.matrix <- PC12.present.matrix.temp[c(PC12.present.convexHull, PC12.present.convexHull[1]), ]  # closed polygon
  PC12.future.matrix <- PC12.future.matrix.temp[c(PC12.future.convexHull, PC12.future.convexHull[1]), ]  # closed polygon
} else if (hull_method == "concave") {
  # Concave hull
  PC12.present.convexHull <- concaveman(PC12.present.matrix.temp, 1)
  PC12.future.convexHull <- concaveman(PC12.future.matrix.temp, 1)
  PC12.present.matrix <- PC12.present.convexHull
  PC12.future.matrix <- PC12.future.convexHull
}
# Add lines to plot
lines(PC12.present.matrix, col="black")
lines(PC12.future.matrix, col="red")

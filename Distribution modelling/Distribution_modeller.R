...
if (cast == "hindcast") {
  ...
  # Define future (CMIP5 or CMIP6) climate model.
  CMIP <- 6
  if (CMIP == 5) {
...
if (species == "Dsyl" && responce_variable_type == "binary") {
  ...
  occurrence_file <- "/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dcarthusianorum/Data/Occurrence_database.csv" # New Dsyl species occurrence data incl. new occurrence data from Italy and the Balkan
}
...
if (responce_variable_type == "frequency") {
  ...
  # Import species SDM for masking. These imported SDMs were/can be generated by this same script.
  pred.bin.current.species <- readRDS("/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dsylvestris/Data/SDM_CentralAlps_forecast_defaultAbsenceRegime/pred.bin.current.CentralAlps.selectVar.optimalThreshold.2021-04-16_16_32_57.rds")
  if (cast == "forecast") {
    if (CMIP == 5) {
      pred.bin.LGM.species <- readRDS("/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dsylvestris/Data/SDM_CentralAlps_forecast_defaultAbsenceRegime/pred.bin.LGM.CentralAlps.selectVar.optimalThreshold.2021-04-16_16_32_57.rds")
    } else if (CMIP == 6) {
      pred.bin.LGM.species <- readRDS("/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dsylvestris/Data/Forecast_CMIP6_runs/Forecast_CMIP6_run11/pred.bin.LGM.CentralAlps.selectVar.optimalThreshold.2025-05-19_17_29_52.rds")
    }
  } else if (cast == "hindcast") {
    pred.bin.LGM.species <- readRDS("/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dsylvestris/Data/CentralAlps_hindcast_goodExample/pred.bin.LGM.clusters.df.CentralAlps.selectVar.optimalThreshold.2021-05-04_11_43_09.rds")
  }
...
if (species == "Dsyl" && responce_variable_type == "binary") {
  ...
  # For CEN gene. 
  # For present-day exclusive projection, we can include longitude and latitude to (partially) control for spatial autocorrelation. In this case, this is justified in that there is strong correlation and high r^2 between longitude and latitide and PC1 and PC2 (from the genomic PCA).
  # When hind-casting to the LGM however, we do not expect (LGM) structure to reflect that of the present-day. However, for cross comparison (and for forecasts where we would expect structure to follow similar patterns), it may still be helpful to include latitude and longitude. Final version used with latitude and longitude.
...
if (cast == "hindcast") {
  ...
  if (CMIP == 5) {
    ...
    climate_CMIP_model_selector <- 1
    clim_CMIP_model_list.names <- c("ENSEMBLE", "CESM1_BGC", "MPI_ESM_MR", "MIROC5", "CMCC_CM", "CESM1_CAM5", "IPSL_CM5A_MR", "ACCESS1_0")
    #clim_CMIP_model_list.data <- c("ENV_RASTERS_CMIP5_EUROPE_ENSEMBLE_ALL.rds", "ENV_RASTERS_CMIP5_EUROPE_CESM1-BGC_RCP45_2061-2080.rds", "ENV_RASTERS_CMIP5_EUROPE_MPI-ESM-MR_RCP45_2061-2080.rds", "ENV_RASTERS_CMIP5_EUROPE_MIROC5_RCP45_2061-2080.rds","ENV_RASTERS_CMIP5_EUROPE_CMCC-CM_RCP45_2061-2080.rds", "ENV_RASTERS_CMIP5_EUROPE_CESM1-CAM5_RCP45_2061-2080.rds", "ENV_RASTERS_CMIP5_EUROPE_IPSL-CM5A-MR_RCP45_2061-2080.rds", "ENV_RASTERS_CMIP5_EUROPE_ACCESS1-0_RCP45_2061-2080.rds")
    clim_CMIP_model_list.data <- c("ENV_RASTERS_CMIP5_EUROPE_ENSEMBLE_ALL_LARGE.grd", "ENV_RASTERS_CMIP5_EUROPE_CESM1-BGC_RCP45_2061-2080_LARGE.grd", "ENV_RASTERS_CMIP5_EUROPE_MPI-ESM-MR_RCP45_2061-2080_LARGE.grd", "ENV_RASTERS_CMIP5_EUROPE_MIROC5_RCP45_2061-2080_LARGE.grd","ENV_RASTERS_CMIP5_EUROPE_CMCC-CM_RCP45_2061-2080_LARGE.grd", "ENV_RASTERS_CMIP5_EUROPE_CESM1-CAM5_RCP45_2061-2080_LARGE.grd", "ENV_RASTERS_CMIP5_EUROPE_IPSL-CM5A-MR_RCP45_2061-2080_LARGE.grd", "ENV_RASTERS_CMIP5_EUROPE_ACCESS1-0_RCP45_2061-2080_LARGE.grd")
    climate_model_CMIP <- clim_CMIP_model_list.data[climate_CMIP_model_selector]
  } else if (CMIP == 6) {
    climate_CMIP_model_selector <- 1
    clim_CMIP_model_list.names <- c("ENSEMBLE", "GFDL-ESM4", "IPSL-CM6A-LR", "MPI-ESM1-2-HR", "MRI-ESM2-0", "UKESM1-0-LL")
    clim_CMIP_model_list.data <- c("ENV_RASTERS_CMIP6_EUROPE_ENSEMBLE_ALL_LARGE.grd", "ENV_RASTERS_CMIP6_EUROPE_GFDL-ESM4_SSP370_2071-2100_LARGE.grd", "ENV_RASTERS_CMIP6_EUROPE_IPSL-CM6A-LR_SSP370_2071-2100_LARGE.grd", "ENV_RASTERS_CMIP6_EUROPE_MPI-ESM1-2-HR_SSP370_2071-2100_LARGE.grd", "ENV_RASTERS_CMIP6_EUROPE_MRI-ESM2-0_SSP370_2071-2100_LARGE.grd", "ENV_RASTERS_CMIP6_EUROPE_UKESM1-0-LL_SSP370_2071-2100_LARGE.grd")
    climate_model_CMIP <- clim_CMIP_model_list.data[climate_CMIP_model_selector]
  }
}
...
for (run in seq(1,num_iterations)) {
  ...
  if (absence_data == FALSE) {
    ## Since we have only presence data, we'll need to generate pseudo-absence data. We do this by first randomly sampling coordinates within the geographic extent we think they can inhabit. From this random sample, we'll then condition pseudo-absences to be a minimum distance away from a presence point.
    ...
    ## This will lead however to pseudo-absence points more often that not falling in the lowlands rather than the mountain peaks, which leads to an unbounded (lower temperature side) in the response curves, and biased representation in LGM projection (where high elevation glacial ice sheets predominate). 
  ...
  if (plot_all == TRUE) {
    # Open a pdf file
    #pdf(paste0("/Users/hl636/Desktop/Corr_",lineage,"plot.pdf"), width = 30, height = 30)
  ...
  eval.rf.cv <- rfcv(trainx = Occurrences.Climate[,c(11:37)], trainy = Occurrences.Climate$Presence, cv.fold = 5)
  if (plot_all == TRUE) {
  ...
  ## And in a random forest model:
  rforest.predVars.form <- as.formula(paste("Presence", paste(pred.var.selection, collapse = " + "), sep = " ~ "))
  rf.prob.predVars <- randomForest(rforest.predVars.form, data=Occurrences.Climate)
  if (plot_all == TRUE) {
    rf.prob.predVars.importance <- importance(rf.prob.predVars)
    print(rf.prob.predVars.importance)
    randomForest::varImpPlot(rf.prob.predVars)
  }
  ...
  rf.prob <- randomForest(rforest.form, data=Occurrences.Climate, importance = TRUE, ntree = 1000)
  #rf.prob <- randomForest(rforest.form, data=Occurrences.Climate, importance = TRUE, ntree = 1000, mtry = 4)
  rf.prob
  ...
  if (responce_variable_type == "frequency")  {
    ...
    # Save rasters
    writeRaster(pred.prob.current.masked, paste0("/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dsylvestris/Data/pred.prob.current.masked.", time_stamp))
    writeRaster(pred.prob.current.masked.CentralAlps, paste0("/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dsylvestris/Data/pred.prob.current.masked.CentralAlps.", time_stamp))
  }
  ...
  if (cast == "hindcast") {
    ...
    ENV_LGM_RASTERS <- stack(climate_model_CMIP)
  }
...
# Output results
if (responce_variable_type == "frequency")  {
  writeRaster(pred.prob.LGM.masked, paste0("/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dsylvestris/Data/pred.prob.LGM.masked.", cast, ".", time_stamp))
  writeRaster(pred.prob.LGM.masked.CentralAlps, paste0("/Users/hl636/Documents/Hirzi/ETHPHD/SDM/Dsylvestris/Data/pred.prob.LGM.masked.CentralAlps.", cast, ".", time_stamp))
}

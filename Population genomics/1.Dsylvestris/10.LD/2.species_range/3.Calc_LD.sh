...
# Load modules
module load gsl/2.7.1 zlib/1.3 gcc/11.4.0
...
N_IND=$(cat /cluster/work/gdc/shared/p936/DSYL_ms_revisions/Recalibrated_and_clippedOverlapped_BAMs/bam_196inds_centralAlps_below1400m.filelist | wc -l)
...
# Run command
# Calculate LD based on genotype likelihoods
# --ignore_miss_data has negligable effect on results
/cluster/work/gdc/shared/p936/DSYL_ms_revisions/ngsLD/ngsLD --n_threads $SLURM_CPUS_PER_TASK --verbose 1 --n_ind $N_IND --n_sites $N_SITES --geno ${prefix}.GLF.gz --probs --pos ${prefix}.pos --max_kb_dist 0 --min_maf 0.25 --extend_out | sort -k 1,1Vr -k 2,2V > ${prefix}.ld
#/cluster/work/gdc/shared/p936/DSYL_ms_revisions/ngsLD/ngsLD --n_threads $SLURM_CPUS_PER_TASK --verbose 1 --n_ind $N_IND --n_sites $N_SITES --geno ${prefix}.GLF.gz --probs --pos ${prefix}.pos --max_kb_dist 0 --min_maf 0.2 --extend_out --ignore_miss_data | sort -k 1,1Vr -k 2,2V > ${prefix}_ignore_miss_data.ld
#eval $CMD

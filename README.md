...
Population genomic analyses including [Poolstat](https://bitbucket.org/wegmannlab/poolstat/wiki/Home), computation of site frequency spectra, F<sub>ST</sub> genome scans using a hidden Markov model, computation of population genetic thetas, F<sub>ST</sub> and theta plots (for candidate regions), candidate annotation tables, GWAS on flowering time, estimation of linkage disequilibria (LD) for focal populations and across the species' range, gene-environmental associations, and isolation-by-distance (IBD).

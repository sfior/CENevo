...
cat Dc.final.28.10.14.contigs.v6.maker.contig.to.scaffold.gff3 | sed '/contig/d' > Dc_trimmed.gff
...
gff2bed < Dc_trimmed.gff > Dc_trimmed.bed
...
cat Dc_trimmed.bed | sed '/maker/d' > Dc_trimmed_others.bed
grep maker Dc_trimmed.bed > Dc_trimmed_maker.bed
...
bedops --merge Dc_trimmed_others.bed  > Dc_trimmed_others_merged_temp.bed
bedops --merge Dc_trimmed_maker.bed  > Dc_trimmed_maker_merged_temp.bed
...
python fix_gff2bed.py Dc_trimmed_others_merged_temp.bed -o Dc_trimmed_others_merged.bed
python fix_gff2bed.py Dc_trimmed_maker_merged_temp.bed -o Dc_trimmed_maker_merged.bed
...
#python addFlankingRegions2BED.py Dc_trimmed_others_merged.bed -f 1000 -o ${output}/Dc_trimmed_others_merged_flanked.bed
python addFlankingRegions2BED.py Dc_trimmed_maker_merged.bed -f 5000 -o ${output}/Dc_trimmed_maker_merged_flanked.bed

### 9. Concatenate the two bed files
#cat ${output}/Dc_trimmed_maker_merged_flanked.bed ${output}/Dc_trimmed_others_merged_flanked.bed > ${output}/Dc_trimmed_flanked.bed
cat ${output}/Dc_trimmed_maker_merged_flanked.bed Dc_trimmed_others_merged.bed > ${output}/Dc_trimmed_flanked.bed

### 10. Sort bed file (https://www.biostars.org/p/64687/). Remember that sorting needs to precede merging.
cat ${output}/Dc_trimmed_flanked.bed | sort -k1,1V -k2,2n -k3,3n > ${output}/Dc_trimmed_flanked_sorted.bed

### 11. Merge again, as there is new potential overlap after the addition of flanking regions.
bedops --merge ${output}/Dc_trimmed_flanked_sorted.bed  > ${output}/Dc_final_temp.bed
cat ${output}/Dc_final_temp.bed | sort -k1,1V -k2,2n -k3,3n > ${output}/Dc_final.bed
...
bedtools complement -i ${output}/Dc_final.bed -g Dcarth_assembly_homozygous.genome > ${output}/Dc_complement.bed

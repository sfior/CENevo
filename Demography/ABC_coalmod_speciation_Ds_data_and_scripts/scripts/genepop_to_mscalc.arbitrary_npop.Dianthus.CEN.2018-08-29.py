...
def make_spinput(spinput_outfile, ms_outfile, locilength, gstats, loci_ordered, pop_order):
	
	# function adjusted for special Dianthus case where all loci have same length !!
	...
	for locus in loci_ordered: # keep loci in order
		...
		locus_length = locilength # special Dianthus case where all loci have same length
...
def make_ms(ms_outfile, out_msdict, loci_ordered, pop_order):
	...
	for locus in loci_ordered:  # keep loci in order
		...
		try: # some loci may not contain any segregating sites either because they were not polymorphic or because the sites were lost in filtering. Need to except that case!
			...
			lines.append("positions: {0}".format(" ".join([str(x) for x in sorted(out_msdict[locus].values()[0].values()[0].keys(), key = int) ]))) # this is probably meaningless
		except IndexError:
	...
	#					print out_msdict[locus][pop][sample].keys()
						for snp in sorted(out_msdict[locus].values()[0].values()[0].keys(), key = int):
...
def make_bpfile (pop_order, locilength, gstats, bpoutfile, nreps, loci_ordered, mu, rho ):
...
#	line2 = "\t".join([str(locilength)]*int(nloci)) + "\n"
	line2 = "\t".join( [ str(locilength) for x in loci_ordered] ) + "\n" # special Dianthus case where all loci have same length
	...
	theta_values = [] # population-mutation rate per locus
	for loc_len in [ int(locilength) for x in loci_ordered]:  # special Dianthus case where all loci have same length
	...
	# intra-locus recombination rate rho 
	# special Dianthus case where all loci have same length
	rho_values = []
	for loc in loci_ordered:
		rho_values.append( str( locilength * rho * 4 * 100000 ) )
...
def main(argv=None):
	...
	# per site per generation per chromosome (NOT the population mutation rate but haploid chromosomes)
	...
	# this value probably very robust within +- 0.5*10**(-9), as Prunus persica has 7.77 and A. thaliana 7.0, i.e. only 4-6% different!
	mu = 7.31*10**(-9)
...
#	locilength = int(args.locilength)
	locilength = 1212 # for CEN case: single lous of this length
	...
	# all these functions produce outputs where the order of populations is the same as in the genepop input
 	make_bpfile (pop_order, locilength, gstats, bpoutfile, nreps, loci_ordered, mu, rho)

 	make_spinput(spinput_outfile, ms_outfile, locilength, gstats, loci_ordered, pop_order)
 	
 	make_ms(ms_outfile, out_msdict, loci_ordered, pop_order) 
